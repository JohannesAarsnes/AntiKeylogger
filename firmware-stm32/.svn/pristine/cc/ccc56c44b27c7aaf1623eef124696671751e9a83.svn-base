...
#ifdef CRC8_MAXIM
  /************************************************************************************
  * Таблица для расчета контрольной суммы CRC8  MAXIM, DALLAS(зеркальная)
  * Полином: (0x131)  x^8+x^5+x^4+1
  *************************************************************************************/
  const uint8_t MAXIM[256] = {	
  ...
  {
    return MAXIM[sum ^ data];
  }
#endif

#ifdef CRC16_ANSI_M
  /************************************************************************************
  * Таблица для расчета контрольной суммы CRC16 ANSI (зеркальная)
  * Полином: (0x18005) x^16+x^15+x^2+1
  *************************************************************************************/
  const uint16_t ANSI[256] = {
  ...
  {
    return ANSI[(sum & 0xff) ^ data] ^ (sum >> 8);
  }	
#endif

#ifdef CRC16_CCITT
  /************************************************************************************
  * Таблица для расчета контрольной суммы CRC16  CCITT (прямая, не зеркальная)
  * Полином: (0x11021 )   x^16 + x^12 + x^5 + 1 
  *************************************************************************************/
  const uint16_t CCITT[256] =
  {
  ...
  uint16_t crc16_CCITT(const uint8_t* data, unsigned int len, uint16_t sum)
  {
    while(len--)
    {
      sum = CCITT[(uint8_t)((sum >> 8) ^ *data)] ^ (sum << 8);  
      data++;
    }
    return sum;  
  }
  ...
  {
    return CCITT[(uint8_t)((sum >> 8) ^ data)] ^ (sum << 8);
  }
#endif

#ifdef CRC16_CCITT_BITWISE	
  //масив нульових байті для отримання кінцевого значення контр. суми
  ...
  {
    return crc16_CCITT_bitwise(ZEROS, sizeof(ZEROS), sum);
  }
#endif

#ifdef CRC32_IEEE_802_3
  /************************************************************************************
  * Таблица для расчета контрольной суммы CRC32  IEEE 802.3 (зеркальная)
  * Полином: (0x104C11DB7)
  *    x^32+x^26+x^23+x^22+x^16+x^12+x^11+x^10+x^8+x^7+x^5+x^4+x^2+x^1+1
  *************************************************************************************/
  const  uint32_t IEEE_802[256] = {
  ...
  uint32_t crc32_IEEE_802(const uint8_t *data, unsigned int len, uint32_t sum)
  {
      while(len--) 
      { 
         sum = IEEE_802[(sum & 0xFF) ^  *data] ^ (sum >> 8);
         data++;
      }
      return sum; 
  }
  ...
  { 
     return IEEE_802[(sum & 0xFF) ^  data] ^ (sum >> 8);
  }
#endif
...
uint16_t bitwise_to_table_init(uint16_t bitwise, uint16_t poly)
{
  ...
  {
    uint8_t bit = init[1 - ((16-1) >> 3)] & (1 << ((16-1) & 0x07));
    for (uint8_t k=0; k<2; k++)
    {
      init[k] = (init[k] << 1) | (init [k+1] >> 7);
      if (bit) 
        init[k] ^= polynom[k];
    }
  }
  
  return init[1] | (uint16_t)init[0] << 8;
}
...
uint16_t table_to_bitwise_init(uint16_t table, uint16_t poly)
{
  ...
  {				
    ...
    for (uint8_t k=1; k; k--) 
    {
      init[k] = (init [k] >> 1) | ((init[k-1] & 0x01) << 7);
    }
    init[0] >>= 1;
    
    if (bit) 
      init[1-((16-1)>> 3)] |= 1 << ((16-1) & 0x07);
  }	
